...
# - use the VERSION as arg of the bundle target (e.g make bundle VERSION=0.1.1)
# - use environment variables to overwrite this value (e.g export VERSION=0.1.1)
VERSION ?= 0.1.3
...
# ghcr.io/dante-lor/spring-boot-operator:$VERSION and ghcr.io/dante-lor/spring-boot-operator-catalog:$VERSION.
IMAGE_TAG_BASE ?= ghcr.io/dante-lor/spring-boot-operator
...
KIND_CLUSTER ?= spring-boot-operator-test-e2e
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name spring-boot-operator-builder
	$(CONTAINER_TOOL) buildx use spring-boot-operator-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm spring-boot-operator-builder
	rm Dockerfile.cross
...
# A comma-separated list of bundle images (e.g. make catalog-build BUNDLE_IMGS=example.com/operator-bundle:v0.1.1,example.com/operator-bundle:v0.2.0).
